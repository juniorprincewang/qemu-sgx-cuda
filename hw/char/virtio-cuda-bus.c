...
{
    VirtIOSerial *vser = VIRTIO_CUDA(vdev);
...
static void virtio_cuda_save_device(VirtIODevice *vdev, QEMUFile *f)
{
    VirtIOSerial *s = VIRTIO_CUDA(vdev);
    VirtIOSerialPort *port;
...
{
    uint32_t i;
    VirtIOSerial *s = VIRTIO_CUDA(opaque);
    VirtIOSerialPort *port;
...
static int virtio_cuda_load_device(VirtIODevice *vdev, QEMUFile *f,
                                     int version_id)
{
    VirtIOSerial *s = VIRTIO_CUDA(vdev);
...
static void virtio_cuda_device_unrealize(DeviceState *dev, Error **errp)
{
...
static void virtio_cuda_device_realize(DeviceState *dev, Error **errp)
{
    ...
    virtio_init(vdev, "virtio-cuda", VIRTIO_ID_CUDA, // VIRTIO_ID_CUDA
                config_size);
...
static void virtio_cuda_class_init(ObjectClass *klass, void *data)
{
    ...
    vdc->realize = virtio_cuda_device_realize;
    vdc->unrealize = virtio_cuda_device_unrealize;
    vdc->get_features = get_features;
    vdc->get_config = get_config;
    vdc->set_config = set_config;
    vdc->set_status = set_status;
    vdc->reset = vser_reset;
    vdc->save = virtio_cuda_save_device;
    vdc->load = virtio_cuda_load_device;
